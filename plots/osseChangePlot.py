...
def secPlot(z, multi):
    ...
    if z==0:
        z0=np.tile(csmc2_foF2["C2_foF2_map"][:,:24],[1,2])
        err1=(csmc2_foF2["C2_foF2_map"][:,24:]-z0)/z0*100
        ERRP[0,:,:]=err1
    elif (z>=1) & (z<=12):
        z0=np.tile(osse_foF2[z-1][:,:24],[1,2])
        err1=(osse_foF2[z-1][:,24:]-z0)/z0*100
        ERRP[z,:,:]=err1
   # xtitle = YY+' '+TITLES[0][flag]
    ...
    fig.layout.annotations[0].update(text=TITLES[z]) #annotations previously controlled by flag 3, titles by tl
